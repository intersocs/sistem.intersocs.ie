...
suite('Analyzer', () => {
    ...
    suite('ProjectConfig componentDir', () => {
        test('setting `componentDir` searches for dependencies in the given ' +
            'directory', () => {
            const foundDependencies = new Set();
            const root = `test-fixtures/analyzer-componentDir`;
            const sourceFiles = ['my-component.js'].map((p) => path.resolve(root, p));
            const config = new polymer_project_config_1.ProjectConfig({
                root: root,
            ...
            return streams_1.waitForAll([analyzer.sources(), analyzer.dependencies()])
                .then(() => {
                chai_1.assert.isTrue(foundDependencies.has(path.resolve(root, 'path/to/some/components/a_component/a_component.js')));
            });
        });
        test('setting `npm: true` searches for dependencies in "node_modules/"', () => {
            const foundDependencies = new Set();
            ...
            return streams_1.waitForAll([analyzer.sources(), analyzer.dependencies()])
                .then(() => {
                chai_1.assert.isTrue(foundDependencies.has(path.resolve(root, 'node_modules/a_component/a_component.js')));
            });
        });
    });
    suite('.dependencies', () => {
