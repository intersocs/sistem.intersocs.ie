...
suite('Project Config', () => {
    suite('ProjectConfig', () => {
        suite('constructor', () => {
            ...
            test('sets entrypoint relative to root when provided', () => {
                const relativeRoot = 'public';
                const absoluteRoot = path.resolve(relativeRoot);
                const config = new ProjectConfig({ root: relativeRoot, entrypoint: 'foo.html' });
                config.validate();
                assert.deepEqual(config, {
                    root: absoluteRoot,
            ...
            test('adds sources relative to root when provided', () => {
                const relativeRoot = 'public';
                const absoluteRoot = path.resolve(relativeRoot);
                const config = new ProjectConfig({ root: relativeRoot, sources: ['src/**/*', 'images/**/*'] });
                config.validate();
                assert.deepEqual(config, {
                    root: absoluteRoot,
                    entrypoint: path.resolve(absoluteRoot, 'index.html'),
            ...
            test('npm option does not override other explicitly set values', () => {
                const config = new ProjectConfig({ npm: true, componentDir: '../some_other_dir/over_here/' });
                config.validate();
                assert.equal(config.npm, true);
        ...
        suite('validate()', () => {
            ...
            test('throws an exception when a fragment does not resolve within root', () => {
                ...
                assert.throws(() => config.validate(), /Polymer Config Error: a "fragments" path \(.*bar.html\) does not resolve within root \(.*public\)/);
            });
            test('throws an exception when entrypoint does not resolve within root', () => {
                ...
                assert.throws(() => config.validate(), /Polymer Config Error: entrypoint \(.*bar.html\) does not resolve within root \(.*public\)/);
            });
            test('throws an exception when shell does not resolve within root', () => {
                ...
                assert.throws(() => config.validate(), /Polymer Config Error: shell \(.*bar.html\) does not resolve within root \(.*public\)/);
            });
            ...
            test('throws an exception when builds property was not an array', () => {
                ...
                assert.throws(() => config.validate(), 'Polymer Config Error: "builds" ([object Object]) expected an array of build configurations.');
            });
            test('throws an exception when builds array contains duplicate names', () => {
                ...
                assert.throws(() => config.validate(), 'Polymer Config Error: "builds" duplicate build name "bundled" found. Build names must be unique.');
            });
            test('throws an exception when builds array contains an unnamed build', () => {
                ...
                assert.throws(() => config.validate(), 'Polymer Config Error: all "builds" require a "name" property when there are multiple builds defined.');
            });
            test('throws an exception when builds array contains an invalid preset', () => {
                ...
                assert.throws(() => config.validate(), 'Polymer Config Error: "not-a-real-preset" is not a valid  "builds" preset.');
            });
        });
    });
    ...
    suite('loadConfigFromFile()', () => {
        ...
        test('creates config instance from config file options', () => {
            const config = ProjectConfig.loadConfigFromFile(path.resolve('test/polymer.json'));
            config.validate();
            const relativeRoot = 'public';
            const absoluteRoot = path.resolve(relativeRoot);
            assert.deepEqual(config, {
        ...
        test('reads a valid config from a file with just {} in it', () => {
            const config = ProjectConfig.loadConfigFromFile(path.resolve('test/polymer-minimal.json'));
            config.validate();
        });
    });
    suite('json validation', () => {
        test('throws good error messages', () => {
            try {
                ...
                assert.deepEqual(e.message, `Property 'lint' is not of a type(s) object`);
                return;
            }
    ...
    suite('toJSON()', () => {
        ...
        test('with full config', () => {
            const config = ProjectConfig.loadConfigFromFile(path.resolve('test/polymer-full.json'));
            assert.deepEqual(JSON.parse(config.toJSON()), {
                ...
                builds: [
                    ...
                    },
                    {
                        ...
                        js: {
                            minify: { exclude: ['js/unminifiable.js', 'js/already-minified.js'] },
                            compile: {
                                exclude: [
